...
Apex is built on top of cmark-gfm, the GitHub-maintained
CommonMark parser. It extends cmark-gfm with additional
syntax support for MultiMarkdown, Kramdown, and Marked's
special features.
...
- **apex_options**: Configuration structure for processor

  modes and features

- **apex_markdown_to_html()**: Main conversion function
- **apex_to_cmark_options()**: Maps Apex options to

  cmark-gfm flags

- **apex_register_extensions()**: Registers cmark-gfm
...
- Various processor modes (commonmark, gfm, mmd, kramdown,
...
- **Extensions**: Pluggable syntax additions (tables,
...
3. **Extension registration** → Attach syntax extensions

   based on mode

**Parsing** → `cmark_parser_feed()` + `cmark_parser_finish()`

5. **AST** → Tree of `cmark_node` structures
6. **Rendering** → `cmark_render_html()` walks AST and
...
- **Block parsing**: Custom block-level syntax (like tables,

  callouts)

- **Inline parsing**: Custom inline syntax (like wiki links,
...
- **footnotes_inline**: `^[inline footnote]` syntax (extends
...
- **marked_special**: `<!--TOC-->`, `<<[include]>>`, page
