...
**Functional with caveats** - Empty cells merge with
previous non-empty cell:
...
Our `is_colspan_cell()` function treats cells as "empty" if
they contain:
...
Currently header rows can participate in colspan if they
have empty cells:
...
**Recommendation**: This is generally undesirable. Headers
should not span.

**Fix needed**: Extend the `is_first_row` skip logic to
colspan processing.
...
The "Basic Table" in comprehensive_test.md shows some
unexpected colspan attributes:

| Issue | Location | Cause |
|-------|----------|-------|
| `colspan="2"` on Tables row, MMD column | Row 3, Col 3-4 |
Unclear - needs investigation |
| `rowspan="3"` on Footnotes row, GFM column | Row 4, Col 2
| Unclear - needs investigation |
| `colspan="2"` on Metadata row | Row 6, Col 2-3 | Unclear -
needs investigation |

**These DO NOT appear when the same table is tested in
...
3. The markdown source itself may have subtle issues
...
**Use well-formed tables** - Ensure columns align properly

2. **Avoid empty cells in headers** - Fill all header cells

   with content

3. **Test in isolation** - If seeing unexpected spans,

   extract the table to a separate file

4. **Check markdown source** - Use `cat -A` to reveal hidden
...
`process_table_spans()` called per table during postprocessing
   ...
   - Sets `user_data` on cells with `colspan="N"`,

     `rowspan="N"`, or `data-remove="true"`
...
`apex_inject_table_attributes()` walks HTML string, matching cells by indices
   ...
   - Injects span attributes or removes cells marked with

     `data-remove`

### Index Tracking

Both AST walker and HTML walker must maintain synchronized
indices:

- **table_index**: Increments for each `<table>` /
...
**Recommendation**: Safe to use with properly formatted
markdown tables. Issues only appear in edge cases with
malformed input.
